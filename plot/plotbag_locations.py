...
bag = rosbag.Bag('/home/alvika-usl/2022-10-20-16-55-49.bag')
...
for topic,msg,t in bag.read_messages(topics=['/dwm1001/tag/tag/position']):
    ...
    time_loc.append(t.to_sec())
    '''
    if(len(time_loc)>=1):
        time_loc[len(time_loc)-1]=t.to_sec()-time_loc[0]
    '''


for topic,msg,t in bag.read_messages(topics=['/dwm1001/tag/tag/to/anchor/AN0/distance']):
    time_a0t0.append(t.to_sec())
    dist_a0t0.append(msg.data)

for topic,msg,t in bag.read_messages(topics=['/dwm1001/tag/tag/to/anchor/AN1/distance']):
    time_a1t0.append(t.to_sec())
    dist_a1t0.append(msg.data)

for topic,msg,t in bag.read_messages(topics=['/dwm1001/tag/tag/to/anchor/AN2/distance']):
    time_a2t0.append(t.to_sec())
    dist_a2t0.append(msg.data)

for topic,msg,t in bag.read_messages(topics=['/dwm1001/tag/tag/to/anchor/AN3/distance']):
    time_a3t0.append(t.to_sec())
    dist_a3t0.append(msg.data)
   
bag.close()
kernel_size = 30
kernel = np.ones(kernel_size) / kernel_size
time_loc_new=[round(time_loc[i]-time_loc[0],2) for i in range(0,len(time_loc))]
...
time_a3t0=[round(time_a3t0[i]-time_a3t0[0],2) for i in range(0,len(time_a3t0))]


print(len(time_a0t0),len(dist_a0t0))
...
dist_a3t0 = np.convolve(dist_a3t0, kernel, mode='same')

#print(len(dist_a0t0),len(dist_a1t0),len(dist_a2t0))
...
for i in range(0,len(pos_x)):
    ...
    #plt.plot(pos_x[i],pos_y[i],color='green',marker="*",linestyle='solid',linewidth=3)
    plt.plot(pos_x[i], pos_y[i], color='green', linestyle='dashed', 
     marker='o',
...
ax_plot(ax3,time_a3t0,dist_a3t0,title='Anchor3-Tag0')

plt.suptitle('Distance_3a1t_t0moving_indoor')
#plt.savefig('./outdoor/Distance_3a1t_t0moving_indoor.png')
plt.show()

anchor_x = [2.600,2.600,0] #anchor coordinates in meters
anchor_y = [2.070,0,0]
anchor = list(zip(anchor_x, anchor_y))


plt.figure()
plt.plot(time_a0t0,dist_a0t0)
plt.show()
#print(time_a0t0)
